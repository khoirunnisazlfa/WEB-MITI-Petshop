...
// Ambil produk yang memiliki kategori wetfood
$query = "SELECT * FROM products WHERE category = 'wetfood' ORDER BY name";
$stmt = $conn->prepare($query);
$stmt->execute();
$wetfood_products = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Fungsi untuk menghasilkan rating secara acak
function generateRandomRating() {
    return rand(3, 5); // Menghasilkan angka antara 3 hingga 5
}
?>
...
<body>
    ...
    <main>
        <div class="center-text">
            <h2>Wet Food Kucing</h2>
            <p>Temukan berbagai pilihan makanan basah kucing dari berbagai merek yang lezat dan bergizi!</p>
        </div>

        <div class="products">
            <?php foreach ($wetfood_products as $product): ?>
